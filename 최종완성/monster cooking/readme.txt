...
통합버전에서 만족도라벨 갱신이 안되는 문제가 있어서 수정함.(func _update_satisfaction_label() 추가)
SUCCESS 변수명의 중복사용으로 오류가능성 있어서 사운드쪽 success 는 Success_sounds 로 수정함.

name 변수명도 godot 기본변수명중 하나라서 중복을 방지하기 위해 new_name(ingredient.gd), card_name(main.gd)로 수정함

배성우님이 말씀해주신 기능안하는 코드문제도 해결함.

J눌러서 가이드북 뜨게 개선함. 이미지는 임시이미지 제작후 적용.

요리를 너무 늦게 완성하면 점수가 -3되는 코드가 작동을 안하는것을 발견하여 수정함.

게임오버 배경화면으로 페이드인하게하고 결과화면이 뜨게 설정함.


2025_11_29 수정사항:
	마지막 완성본에서 여전히 데몬이 안나오는 문제발견 후 수정
	
	버튼연타 방지를 위해 조합하기 버튼은 몬스터 교체시간과 동일하게 딜레이를 추가했고
	리롤버튼은 횟수가 남아있을 경우에만 누를 수 있게 수정.
	
	저번 수정사항과 마찬가지로 name 변수명은 오류를 불러일으 킬 수 있기때문에 이번에 추가된 
	if deck.is_empty():
			# 덱 리필 로직
			deck.clear()
			for name in card_textures.keys(): 
				deck.append(name)
				deck.append(name)
			deck.shuffle()
			
			해당 부분에서 name를 key로  바꿈
