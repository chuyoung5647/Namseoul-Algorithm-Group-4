...
# --- ProgressBar & 내부 라벨 (씬에 있는 이름 그대로) ---
...
func _process(delta):
	...
	if not game_over:
		...
		# 10초 교차 시 사운드 재생 (델타로 한 프레임 지나쳤을 수 있으니 교차 검사)
...
func _on_draw_button_pressed():
	...
	if not deck.is_empty() and hand_cards.size() < 5:
		var name = deck.pop_front()
		var card = IngredientCardScene.instantiate()
		player_hand.add_child(card)
		card.setup(name, card_textures[name])
...
func process_satisfaction_logic(success: bool, elapsed_time: float):
	var change = _calculate_satisfaction_score(success, elapsed_time)
