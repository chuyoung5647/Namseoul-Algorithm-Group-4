...
@onready var monster_option = $Monster #몬스터의 이미지변경용 변수
...
func _ready():
	get_new_order()
	monster_imageset(current_order)
	deck.shuffle()
...
#current_order변수를 받아서 몬스터의 이미지 변경
func monster_imageset(order):
	monster_option.set_monster(order)
